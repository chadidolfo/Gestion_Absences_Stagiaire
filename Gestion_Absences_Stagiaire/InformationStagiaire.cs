...
{
    public partial class InformationStagiaire : Form
    {
        public InformationStagiaire()
        {
            InitializeComponent();
        }
       
        private void InformationStagiaire_Load(object sender, EventArgs e)
        {
            ListeStagaire();
        }
        private void ListeStagaire()
        {
            ...
            dgvAfficher.AllowUserToAddRows = false;
            dgvAfficher.AllowUserToDeleteRows = false;
            dgvAfficher.AllowUserToResizeColumns = false;
            dgvAfficher.ReadOnly = true;
            dgvAfficher.AllowUserToResizeRows = false;
            ...
            if (rd.HasRows == true)
            {
                this.dgvAfficher.Rows.Clear();
                while (rd.Read())
                {
                    this.dgvAfficher.Rows.Add(rd[0], rd[1], rd[2], rd[3], rd[4], rd[5], rd[6], rd[7], rd[8], rd[9], rd[10], rd[11]);
                }
                GLB.cnx.Close();
            }
        ...
        private void btnAfficher_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (GLB.dr.HasRows)
                {
                    while (GLB.dr.Read())
                    {
                        this.dgvAfficher.Rows.Add(GLB.dr[0], GLB.dr[1], GLB.dr[2], GLB.dr[3], GLB.dr[4], GLB.dr[5], GLB.dr[6], GLB.dr[7], GLB.dr[8], GLB.dr[9], GLB.dr[10], GLB.dr[11]);
                    }
                }
                else
